...
// ——— Component
export default function FrontofficeManuals({ manuals }) {
  // —— Hash Router voor statische hosting
  // Route: { name:'home' } | { name:'truck', id:string }
  const parseHash = () => {
  ...
  const navigateTruck = (id) => { window.location.hash = `/truck/${id}`; };

  // Data state
  /** @type {[Manual[], Function]} */
  const [all, setAll] = useState(manuals || DEMO_TRUCKS);
  const [selectedId, setSelectedId] = useState(() => (manuals || DEMO_TRUCKS).find((t) => t.actief)?.id || (manuals || DEMO_TRUCKS)[0]?.id || "");
  useEffect(() => {
    if (route.name === 'truck') {
      const exists = all.some(t => t.id === route.id);
      if (exists) setSelectedId(route.id);
    }
  }, [route.name === 'truck' ? route.id : undefined, all]);
  ...
  const [opbouwDone, setOpbouwDone] = useState({});
  const [afbouwDone, setAfbouwDone] = useState({});
  const [packDone, setPackDone] = useState({});
  const [incident, setIncident] = useState("");

  // Persistente admin-auth
  useEffect(() => {
  ...
  function updateAt(key, value) {
    setAll((arr) => {
      const copy = [...arr];
      const idx = copy.findIndex((t) => t.id === truck.id);
      if (idx >= 0) copy[idx][key] = value;
      return copy;
    });
  }
  function onSave() {
  ...
  function addTruck() {
    const id = `truck-${uid()}`;
    const novo = { id, naam: "Nieuwe truck", actief: true, infra: { stroom: "" }, menu: [], apparatuur: [], opbouw: [], afbouw: [] };
    setAll((arr) => [...arr, novo]);
    setSelectedId(id);
    navigateTruck(id);
  }
  ...
  function removeMenuItem(id) { updateAt("menu", truck.menu.filter((m) => m.id !== id)); }
  // Apparatuur CRUD
  function addApparatuur() { updateAt("apparatuur", [...truck.apparatuur, { id: uid(), naam: "Nieuw apparaat" }]); }
  function removeApparatuur(id) { updateAt("apparatuur", truck.apparatuur.filter((a) => a.id !== id)); }
  // Checklist CRUD
  function addChecklist(section) { updateAt(section, [...(truck[section] || []), { id: uid(), text: "Nieuwe stap", required: false }]); }
  function removeChecklist(section, id) { updateAt(section, (truck[section] || []).filter((c) => c.id !== id)); }
  // Paklijst CRUD
  function addPackItem() { updateAt("paklijst", [...(truck.paklijst || []), { id: uid(), naam: "Nieuw item" }]); }
  function removePackItem(id) { updateAt("paklijst", (truck.paklijst || []).filter((p) => p.id !== id)); }
  ...
  // Admin toggle intent
  function handleAdminToggleIntent(next) {
  ...
  useEffect(() => {
    ...
    console.assert(Array.isArray((DEMO_TRUCKS[0]).opbouw), "Opbouw lijst ontbreekt");
    console.assert(Array.isArray((DEMO_TRUCKS[0]).afbouw), "Afbouw lijst ontbreekt");
    console.assert(Array.isArray((DEMO_TRUCKS[0]).menu), "Menu lijst ontbreekt");
    console.assert(Array.isArray((DEMO_TRUCKS[0]).paklijst || []), "Paklijst ontbreekt (optioneel)");
    const prev = window.location.hash; window.location.hash = '/';
    const parsedHome = parseHash().name; window.location.hash = prev;
        ...
        <CardContent className="space-y-3">
          ...
          {truck.menu?.map((m) => (
            <div key={m.id} className="flex items-center gap-3 border rounded-xl p-3">
              {m.fotoUrl && <img src={m.fotoUrl} className="w-16 h-16 object-cover rounded-lg" alt="" />}
...
function PacklistSection({ items, done, onToggle, onExportCSV }) {
...
// ——— Admin secties
function ChecklistAdmin({ title, items, onAdd, onRemove, onChange }) {
  return (
    <Card>
...
function ServiceAdmin({ menu, apparatuur, onMenuChange, onAppChange, onAddMenu, onRemoveMenu, onAddApp, onRemoveApp }) {
...
function PacklistAdmin({ items, onChange, onAdd, onRemove }) {
  return (
    <Card>
